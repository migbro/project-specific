...
if len(sys.argv) == 1:
    sys.stderr.write('Usage: ' + sys.argv[0] + '{container}{object}{bnid list}{output file name}{cont2}\nn')
    exit(1)
    ...
    if qc_list is not None:
        for fname in qc_list:
            ...
            try:
                ...
                continue
            try:
                cov = qc_data['coverage']['V3_capture_t1']['average']
                bdict[bnid]['cov'] += float(cov)
                bdict[bnid]['ct'] += 1
            except:
                try:
                    cov = qc_data['coverage']['V2_capture_t1']['average']
                    bdict[bnid]['cov'] += float(cov)
                    bdict[bnid]['ct'] += 1
                except:
                    try:
